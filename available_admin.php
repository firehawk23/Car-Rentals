...
function print_cars($conn,$sql){
    echo "<div style=margin-left:3%;>";
echo "<h2>Available Cars</h2>";
echo '<a href="adminhomepage.php"><i class="fa fa-reply" style="font-size:25px;"></i></a>';
...
while($row=$result->fetch_assoc()){
    ...
    <hr/>
    <input type=submit value=Edit name=book class="example_a">
    </form>
...
echo "<div style=\"display:inline-block;width:60%;border-left:1px solid black;\">";
if($cid!="all"){
 print_cars($conn,$sql_act." and cid='$cid'");
 echo "</div>";
}
else{
    print_cars($conn,$sql_act);
...
header("Location:admin_edit.php?carnum=$carnum");                                                          
}
